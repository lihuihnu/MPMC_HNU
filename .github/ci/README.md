...
Linux 科学测试默认使用 GitHub 官方 `ubuntu-24.04`，Windows/macOS 保留官方平台。私有 `mpmc_hnu` 仅用于明确白名单中的长时/稳定硬件 Gate；当前仅 CPA performance audit 使用私有 runner。新增私有 runner 消费必须同时更新治理白名单并给出运行时间或稳定硬件需求证据。`Required CI result` 汇总失败/取消，不将 skipped 当作已跑过测试。分支保护不在此次修改范围内。
