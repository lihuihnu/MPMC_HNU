...
> **当前状态：AD 算术与初等函数增量。** 已提供独立的 C++20 `mpmc::ad::Dual<T, N>`、常见初等函数、独立增量测试入口与官方 runner 工作流；功能边界和使用方法见 [AD 模块说明](modules/ad/README.md)。热力学、闪蒸及其他模块仍为规划，没有已验证物理模型或性能达标结论；测试结果以具体提交的 GitHub Actions 日志为准。开发约束见 [AGENTS.md](AGENTS.md)。
...
首个代码增量已提供固定维数一阶前向 `Dual<T, N>`，只处理值/导数语义、种子与基本算术。现已在独立数学头文件中增加常见初等函数；逐函数定义域、异常与增量测试入口见 [初等函数契约](modules/ad/math.md)。使用与功能边界见 [AD 模块说明](modules/ad/README.md)。后续按实际需求增加其他数学函数、动态导数维数和 Jacobian 提取等能力。反向模式（VJP）、稀疏传播、高阶导数与表达式优化作为后续能力，每项均需需求与代价审计，不在第一次实现中一次性承诺完成。[E6]
