...
| M3：PR 与两相基线 | 已提供有序组分/PR76 参数契约及纯组分 a(T)、b；PR 物性、稳定性与两相闪蒸待逐项实现 | 来源明确的适用体系，守恒、相平衡与稳定性回归。 |
...
文档初始化阶段没有创建代码或 CI。当前 AD、热力学数据契约和 PR76 纯组分数值核均有独立增量测试入口与官方 runner 工作流；尚未实现混合物性、EOS 根或闪蒸计算，未创建许可证或占位目录。具体执行结果查阅对应 PR/提交的 Actions 日志，不能将配置了工作流视为测试已通过。完整审计、提交与验证规则以 [AGENTS.md](AGENTS.md) 为准。
