...
> **当前状态：AD 求导能力与热力学数据契约。** 已提供独立的 C++20 `mpmc::ad::Dual<T, N>`、常见初等函数、`value_and_jacobian`、分块 `value_and_jacobian_runtime<K>`、独立增量测试入口与官方 runner 工作流；功能边界和使用方法见 [AD 模块说明](modules/ad/README.md)。已新增[有序组分与 PR76 数据契约](modules/thermodynamics/README.md)，尚无 EOS 或闪蒸计算；没有已验证物理模型或性能达标结论；测试结果以具体提交的 GitHub Actions 日志为准。开发约束见 [AGENTS.md](AGENTS.md)。
...
| 构建与测试入口 | CMake 3.21+、CMake Presets、CTest | 已提供独立 `mpmc::ad`、数据契约 `mpmc::thermodynamics` 目标及各自增量测试入口；闪蒸待开发。[E1] |
...
下表描述目标职责；**当前 AD 已有计算代码，`modules/thermodynamics` 已有有序组分与 PR 数据契约，其他模块尚未创建**。只在对应增量需要时创建文件与构建目标，不预生成空模块、占位实现或插件框架。
...
当前已实现的[PR76 数据契约](modules/thermodynamics/README.md)按稳定组分 ID 绑定参数并形成运行期有序快照；公共来源、单位与身份层供后续 SW/CPA 复用，不将 PR 的常数对称 kij 规则强加给它们。本轮不计算 EOS。
...
| M3：PR 与两相基线 | 已提供有序组分/PR76 参数契约；PR 物性、稳定性与两相闪蒸待逐项实现 | 来源明确的适用体系，守恒、相平衡与稳定性回归。 |
...
文档初始化阶段没有创建代码或 CI。当前 AD 与热力学数据契约增量已分别加入最小必要构建、测试与官方 runner 工作流；尚未实现 EOS 或闪蒸计算，未创建许可证或占位目录。具体执行结果查阅对应 PR/提交的 Actions 日志，不能将配置了工作流视为测试已通过。完整审计、提交与验证规则以 [AGENTS.md](AGENTS.md) 为准。
