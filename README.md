...
| 服务通信 | Protocol Buffers + gRPC；浏览器通过 gRPC-Web 适配层接入 | 契约先行、消息版本化；传输对象与计算核心类型分离。[E3][E4][E9] |
...
| CPA（Cubic-Plus-Association） | 已实现 explicit-site-pair CPA 参数/缔合/相性质、minimum-Gibbs PT stability、VLE、maximum-three-phase 编排与统一 PT backend；两相路径已有可追溯 methanol(2B)-water(4C) 物理回归，三相目前只有 synthetic structural validation，尚无兼容的物理 VLLE oracle。[S3] |
...
### 6.1 当前可用：PR76、SW92 与 CPA PT flash 后端
...
[泡点／露点邻域回归](modules/flash/boundary_regression.md) 和[极近露点门槛审计](modules/flash/dew_limit_audit.md) 记录有限采样范围、独立高精度二元参考与相消失边界；原边界文档中的 TPD 停滞是历史记录，后续修复依据与回归见 [TPD 模块说明](modules/flash/README.md)。这些是软件和模型数值验证，不是全局稳定性证书。SW92 Profile-C 的物理三相验证还包括 [Sample-6 authoritative completion audit](modules/flash/sw92_authoritative_three_phase_audit.md) 与 [n-butane/H2O 实验三相线](modules/flash/sw92_three_phase_experimental_validation.md)；CPA 已有 [methanol-water associating two-phase physical validation](modules/flash/cpa_physical_validation.md)，但尚无兼容的 physical three-phase oracle。当前不提供生产泡点／露点 API、CPA flash sensitivity/physics closure、PR76 standalone 单相 physics closure、validated LV/LL morphology classifier、salt-inventory conservation、pore-volume accumulation、production conservation residual/flux、网格/离散/time stepping/global Jacobian/流动求解，或跨泡露点/相消失/临界/family/root/topology switching 的闪蒸导数。
...
| M8：应用与结果 | **已提供 model-neutral PT service boundary v1、`mpmc.runtime.v1` Protobuf/gRPC-Web mapping，以及 discovery 驱动的动态 backend/component 前端**；后续接入 C++ process host、部署、可视化与结果导出 | C++ service contract/configuration/three-backend discovery/error regression 与浏览器 Protobuf binary round-trip、request preservation、variable phase、indeterminate/service/RPC 分层、deadline/cancel/no-retry、生成一致性和 UI build 已有 focused 入口；真实跨进程端到端、server-side payload/concurrency/resource gate 尚待 process-host 增量。 |
