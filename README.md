...
[Model-neutral PT service boundary v1](modules/runtime/README.md) 已在应用层建立：运行期注册 PR76/SW92/CPA configured backend，发现其有序组分 inventory 与 capability，以稳定 ID 接收请求，并纯投影 provenance、variable-cardinality accepted phase set、transition evidence 及 indeterminate/error 状态。该层不重跑 EOS、守恒、逸度、TPD 或相数逻辑；wire transport 与 frontend migration 尚未接入。
...
`modules/runtime` 现已实现上述 transport-neutral C++ service boundary；依赖方向为 `runtime -> flash -> thermodynamics`，不形成反向依赖。它不是 Protobuf/gRPC server，也不改变现有前端仍未连接真实计算服务的状态。
...
| M8：应用与结果 | **已提供 model-neutral PT service boundary v1 的 runtime inventory/capability/request/result/provenance/variable-phase/indeterminate-error C++ 契约**；后续接入 wire API、前端、可视化与结果导出 | service contract/configuration/three-backend discovery 与错误路径已有 focused regression；进入前端计算连接前仍需 Protobuf/gRPC-Web 往返、取消/超时/资源边界及展示一致性验证。 |
