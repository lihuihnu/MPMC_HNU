...
温度与温度种子沿用纯组分的数学域检查：T>0 且有限，AD 种子有限。参数快照声明的经验温度区间是 advisory，而不是混合核的执行门禁；有限正温度落在区间外时仍继续计算代数外推。调用方可从 `parameters().applicability()` 取得声明范围；由于本核没有压力输入，不能单独把一个状态判成完整 T/p 意义上的 `inside_declared_bounds`，应由拥有压力的 PT 层使用 `assess(T,p)` 给出 `inside/outside/unknown` 告知。本核本身也不能据此证明相态或物理准确性。
...
| `domain_error` | 组成、种子或余量非法；完整组成和不合格；温度/温度种子违反纯组分数学域（如 T<=0 或非有限）；多组分有 a_i<=0。 |
| 已声明经验温区外 | **不抛异常**；继续计算代数外推，并由上层通过 `Applicability` 明确报告 `outside_declared_bounds`，不得静默视为已验证适用。 |
...
新增独立工程 `tests/thermodynamics/pr76_mixture`，15 个 CTest：二元解析比例、完整梯度、约化梯度、方向链式法则、零组成、4 组分全部 24 排列、运行期 1→5→2→4→1、归一化、温度与种子、pair 规则、主平方根、binary32 零吸引边界、所有权与失败恢复、大小尺度、公共头。其中 13 项覆盖 float/double/long double；零吸引边界采用明确的 float 舍入输入，公共头项为普通 double。24 排列是一个测试条目的子情形，不额外计数。`temperature_and_seeds` 现明确区分数学非法温度与声明范围外外推：前者继续作为错误，后者保持可计算并要求适用性状态可见。
