...
设 `q=1+kappa*(1-sqrt(T/Tc))`，实现 `a=a_c*q*q`。数学温度域为有限 `T>0`；Tc/Pc/omega 作为常量模型参数，不参与本轮求导。参数快照声明的经验温度区间是 **advisory 元数据**，不是本核的数学执行门禁：有限正温度即使落在区间外仍可继续计算。完整 T/p 适用性由拥有压力状态的调用层通过 `applicability().assess(T,p)` 区分 `inside_declared_bounds`、`outside_declared_bounds` 或 `unknown`；本层没有压力输入，不能单独证明一个状态完整位于已声明 T/p 适用范围内。

原文叙述其 alpha 拟合使用从正常沸点到临界点的蒸气压数据。这不是对所有组分/温度的普适精度保证。本内核在未知经验范围或超临界温度上执行平方表达式，是明确的**代数延拓**而非物理验证。特别是 q<0 时，平方仍可计算，但不再声称满足非负主平方根 `sqrt(alpha)=q` 的原式形式；不自动裁剪 q、取绝对值、修改 alpha 或伪造有效性。需要禁止外推时，应由更上层策略根据可追溯的适用性规则显式拒绝；本核不把经验适用范围外等同于数值非法。
...
| 情形 | 行为 |
| --- | --- |
| T<=0、NaN/Inf 温度、非有限 AD 种子 | `domain_error`；不修改输入或准备对象 |
| 有限正温度落在已声明经验温区外 | 继续计算代数外推；调用层应通过 `Applicability` 报告 `outside_declared_bounds`，不得静默宣称已验证适用 |
...
包含原文系数与临界温度恒等式、负/零/高 omega、任意/零种子与温度复合链式法则、固定 2x1 Jacobian，以及运行期组分快照和分块接口的必要集成。运行期测试在同一进程以 1->5->2->4 维输入消费新快照，5 维时包含尾块；不同组分分别拥有独立温度仅为导数矩阵测试，并非相平衡物理模型。来源寿命、温度域、已知/未知适用性、超范围转换与大/小尺度也有对应检查。`declared_bounds` 现同时验证：经验边界内外都可执行代数求值，而边界外状态必须保持可查询的 `outside_declared_bounds` advisory，不把外推升级成已验证适用。
