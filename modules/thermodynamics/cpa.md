...
- the simplified radial-distribution expression `g = 1/(1 - 1.9 eta)`, with `eta = b*rho/4`;
- explicit site-pair association parameters. No hidden cross-association combining rule is selected in this profile.

Primary CPA anchor: G. M. Kontogeorgis, E. C. Voutsas, I. V. Yakoumis and D. P. Tassios, *An Equation of State for Associating Fluids*, Industrial & Engineering Chemistry Research 35 (1996) 4310-4318, DOI `10.1021/ie9600203`. That work defines CPA as an SRK physical term combined with a perturbation-theory association contribution and fits five pure-compound parameters for associating compounds.

A later multicomponent anchor is G. M. Kontogeorgis, I. V. Yakoumis, H. Meijer, E. Hendriks and T. Moorwood, *Multicomponent phase equilibrium calculations for water-methanol-alkane mixtures*, Fluid Phase Equilibria 158-160 (1999) 201-209, DOI `10.1016/S0378-3812(99)00060-6`.

The profile identity names the actual simplified `1.9 eta` radial-distribution expression instead of assigning it to a publication year. This avoids implying that a year alone uniquely identifies a CPA implementation.
...
Every numerical datum is validated with repository provenance and the owning parameter snapshot retains the selected pure, binary and association source records. Synthetic data require `DataPolicy::allow_synthetic_tests`.

Absence of an association site-pair record means that interaction is disabled by this configured profile. It does **not** mean that an unknown cross-association parameter is silently inferred. If a future model profile adopts CR-1, Elliott, or another combining rule, that rule must receive its own profile/convention and validation.

## Association kernel

For supplied `T`, molar density `rho`, composition `x`, and a configured parameter set, `solve_cpa_association(...)` solves the site-fraction equations
...
For the configured simplified radial distribution,
...
4. component-permutation, source-retention and analytic structural regression.
...
The next CPA gate should derive and independently validate the full phase chemical-potential/fugacity kernel together with the density/root solve before connecting CPA to the generic PT stability and max-three-phase backend.
