...
在预测下降已不大于两点舍入保护量之和时，**改用残差进展规则替代 Armijo**，允许 `D_new<=D+guard_sum` 且驻点残差下降严格超过 `0.1*(alpha/s)*旧残差` 的步，避免正常舍入导致停滞；该规则**不是收敛判据**，最终仍必须通过原残差阈值。物性失败可拒绝后缩步恢复。预算统计包含参考态、拒绝步与失败调用；资源耗尽不伪造收敛。
...
第二轮 run `34308625726` 三平台均通过编译，原回溯恢复项与新增舍入保护项通过；三平台均仅 `ideal_search` 未通过（21/22）。互斥分支修复了原缺陷，但固定 10% 残差下降要求忽视方向缩放：在理想 trace 情形，残差可为数百，受限对数步每次最多改变 4，目标加权下降已小于舍入保护量，实际残差仍明确下降，却不可能在一步内下降 10%。理想模型的组分间残差差值满足精确关系 `(q_i-q_j)_new=(1-alpha/s)*(q_i-q_j)_old`；加权中心也随组成改变，不能把该关系直接写成整个残差向量同比缩放。这说明进展要求必须考虑实际步长，而不能强制每步固定比例下降。

因此最终舍入区判据改为 `old_residual-new_residual > 0.1*(alpha/s)*old_residual`，同时保持原目标值保护；仍严格拒绝空步与增残差步，下降可分辨区域继续使用原 Armijo。追加受限理想 trace 的接受／空步拒绝检查，保留所有原理想 trace 输入、预算、误差阈值和驻点收敛条件。本修正改变的是算法进展尺度，不是降低最终解的精度要求。上述固定 10% 规则保留在历史说明中，不再是最终实现。
