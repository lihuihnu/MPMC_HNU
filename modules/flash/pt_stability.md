> 本页保留原 TPD 增量及 PR #12 修复的契约、审计和验证记录；“本轮”“下一步”等描述按当时范围理解。当前模块能力与后续实现见 [闪蒸模块导航](README.md)。
...
在预测下降已不大于两点舍入保护量之和时，**改用残差进展规则替代 Armijo**：要求 `D_new<=D+guard_sum`、实际步长 `alpha/s>0` 且 `新驻点残差<旧驻点残差`。必须是可表示的严格下降，不接受空步或增残差；不再强加未经证明的残差收缩率下界（见第 8 节）。该规则**不是收敛判据**，最终仍必须通过原残差阈值。物性失败可拒绝后缩步恢复。预算统计包含参考态、拒绝步与失败调用；资源耗尽不伪造收敛。
...
因此当时的舍入区判据改为 `old_residual-new_residual > 0.1*(alpha/s)*old_residual`，同时保持原目标值保护；仍严格拒绝空步与增残差步，下降可分辨区域继续使用原 Armijo。追加受限理想 trace 的接受／空步拒绝检查，保留所有原理想 trace 输入、预算、误差阈值和驻点收敛条件。本修正改变的是算法进展尺度，不是降低最终解的精度要求。上述两种比例规则保留在历史说明中；当前低曲率修正见第 8 节。
...
## 8. PR #12：低曲率舍入区回溯停滞的定点修正

以上初始增量历史保留。汽液入口及共同参考见 [pt_split.md](pt_split.md)，PR #11 的只读二元参考见 [boundary_regression.md](boundary_regression.md)。本轮基线为 PR #11 的 `7d56ae774d472e66e06c30b60696a74382cece65`；先提交审阅 COMMENT `5152114311`，后增加独立回归，再以官方失败轨迹记录生产修复前审计 comment `5599376086`。这是原实现者复核，不是第三方审稿；不批准或合并前序 PR。

### 根因与最小复现

设 R=max|r_i|，实际对数步长 t=alpha/s。在平坦的光滑区域，可能有 R_new=(1-lambda*t)R+高阶项，且 0<lambda<0.1。旧舍入区条件要求 R-R_new>0.1*t*R，即强加不存在的模型无关曲率下界。缩小 t 同时缩小真实进展与要求，不能在一阶上修复这一障碍。Gibbs-Duhem 所给的目标下降方向，不保证最大残差以这个比例下降。

独立严格凸反例为二元正则溶液 g=x log x+(1-x)log(1-x)+chi*x*(1-x)，chi=1.95，z=(.5,.5)，w=(.500001,.499999)。g''>=4-2chi>0，而 logit 坐标在 x=.5 的局部残差收缩率为 1-chi/2=.025。原算法拒绝真实约 2.5% 的完整步进展。这是单独推导的数学夹具，不是新的实际流体参数。

旧生产实现的官方 red run `34333018098`，Clang job `102405791836` 编译成功、42/47项通过，恰有五项新增强制成功回归失败；原31相分裂、10边界及新增预算项通过。独立参考步骤因 CTest 失败被跳过，不能写成该轮参考已通过。失败点及一次完整对数步的诊断摘录：

| 参考类型 / p（Pa） | 旧驻点残差 R | 一步诊断 R_new | 进展比例 | 预测下降 / 两点 guard 和 |
| --- | ---: | ---: | ---: | --- |
| 初始 / 6479103.4015238024 | 8.9711673613e-7 | 8.4087888190e-7 | 6.2687% | 3.4493e-13 / 3.8707e-13 |
| 最终 / 6437708.6063012648 | 8.8678242382e-7 | 8.2069271626e-7 | 7.4528% | 3.6283e-13 / 3.9356e-13 |
| 最终 / 6478689.4535715776 | 8.9593632611e-7 | 8.3965836200e-7 | 6.2815% | 3.4427e-13 / 3.8713e-13 |
| 最终 / 6479099.2620442808 | 3.3780444709e-7 | 3.1663082597e-7 | 6.2680% | 4.8905e-14 / 3.8707e-13 |

CO2／甲烷固定 T=220K、z=(.30,.70)，参数和参考值完全复用 PR #11。表内旧点来自实际返回的 C++ trial；下一点由测试在该旧点上额外调用一次物性诊断得到，不冒充已接受迭代，也不计入求解器的 evaluations。这里只记录有限摘要，不输出每步物性日志。

### 修正、边界及验证选择

只修改共享 `stability_accept_step` 的舍入区分支为第 4 节的严格残差进展规则；可分辨下降仍走原 Armijo，两个分支保持互斥。保留 helper 签名和公共接口；未改 TPD 求值／参考构造、PR76／RR／相分裂源码、初值、零／trace、非光滑、负证据优先级和资源统计。没有因改动初始参考而重新定标共同参考。

严格数值下降不等于可信误差包络，也不保证任意多组分问题的最大残差会沿梯度下降。非常平坦或物性噪声主导区域仍可能耗尽预算或无法进展，必须返回 indeterminate；不能因接受若干小步就宣称驻点或全局稳定。原驻点1e-8、负TPD1e-10、512步、32次回溯、100000次物性预算均保持。两相1e-11逸度、1e-12绝对／1e-10相对守恒、相分率1e-10及原独立前向比较预算也不变。

新增六项独立 CTest 在 `tests/flash/pt_split/stagnation_test.cpp`：空步／增残差／目标guard／正常Armijo与低曲率进展，三种严格凸正则溶液，三个初始CO2状态，三个显式共同切平面状态，三个默认初值完整闪蒸状态及组分逆序，三阶段预算隔离。完整闪蒸项必须通过最终复核，不能继续把已记录的 indeterminate 当通过；相分率、组成、Z和重新计算的守恒／逸度同时校验。失败回归先提交，生产修复不修改它们或旧测试。

因共享稳定性代码变化，本轮需要原22项稳定性、原31项相分裂、PR #11的10项边界和新增6项，共69个不同条目；使用原三种官方托管编译配置。相分裂工作流运行47项，稳定性工作流运行22项，并保留原独立Decimal80及边界96位核验。未更改参考字面值或生成器、不运行无关AD／thermo旧套件、不引入第三相／含水模型／新依赖。正式执行结果以具体提交的官方CI为准；本节先记录实际red结果与修复设计，不预写通过。

[R1] 的 TPD 及归一化逐次代入式 (21)–(24) 在本轮再次查看 PDF 第3–4页；该论文不包含本项目的舍入接受规则。上述曲率反例和最小修复独立推导，不冒称文献原算法复现。
