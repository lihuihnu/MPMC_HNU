#ifndef MPMC_FLOW_PR76_METHANE_ETHANE_PROPANE_PROPERTIES_HPP
#define MPMC_FLOW_PR76_METHANE_ETHANE_PROPANE_PROPERTIES_HPP
...
namespace pr76_mep_property_detail {
...
inline constexpr std::array<double, 3>
    nist_molar_mass_kg_per_mol{
        0.0160425,
        0.0300690,
        0.0440956};

/// NIST Chemistry WebBook SRD 69 critical molar volumes.
///
/// methane: 0.09860 L/mol (Ambrose & Tsonopoulos, 1995)
/// ethane: reciprocal of 6.87 mol/L critical density
///         (Ambrose & Tsonopoulos, 1995)
/// propane: 0.200 L/mol (Ambrose & Tsonopoulos, 1995)
inline constexpr std::array<double, 3>
...
inline constexpr std::array<double, 14>
    nist_cp_temperature_k{
        298.15,
        300.0,
        400.0,
        500.0,
...
/// NIST SRD 69 recommended ideal-gas Cp [J/(mol K)].
/// The common 298.15--1500 K window is deliberately narrower than some
/// individual component tables, so one mixture contract has one explicit range.
inline constexpr std::array<
    std::array<double, 14>,
    3>
    nist_ideal_gas_cp_j_per_mol_k{{
        {
            35.69, 35.76, 40.63, 46.63, 52.74, 58.60, 64.08,
            69.14, 73.75, 77.92, 81.68, 85.07, 88.11, 90.86},
        {
            52.49, 52.71, 65.46, 77.94, 89.19, 99.14, 107.94,
            115.71, 122.55, 128.55, 133.80, 138.39, 142.40, 145.90},
        {
            73.60, 73.93, 94.01, 112.59, 128.70, 142.67, 154.77,
            165.35, 174.60, 182.67, 189.74, 195.85, 201.21, 205.89}}};
...
inline void require_finite(
    ...
    if (!std::isfinite(primal) ||
        ...
        throw std::range_error(
            std::string{
                "mpmc::flow::PR76 methane/ethane/propane property: "} +
            name +
            (positive
...
inline void validate_composition(
    ...
    if (composition.size() !=
        ...
        throw std::invalid_argument(
            "mpmc::flow::PR76 methane/ethane/propane property requires exactly three components");
    }
    ...
    for (const auto& fraction : composition) {
        ...
        if (!std::isfinite(primal) ||
            ...
            !(primal < 1.0)) {
            throw std::invalid_argument(
                "mpmc::flow::PR76 methane/ethane/propane property requires positive-support composition");
        }
    ...
    if (std::abs(sum - 1.0L) >
        tolerance) {
        throw std::invalid_argument(
            "mpmc::flow::PR76 methane/ethane/propane composition must sum to one");
    }
}
...
integrated_nist_cp(
    ...
    const double lower =
        nist_cp_temperature_k.front();
    const double upper =
        nist_cp_temperature_k.back();
    if (!std::isfinite(primal) ||
        primal < lower ||
        primal > upper) {
        throw std::out_of_range(
            "mpmc::flow::NIST ideal-gas enthalpy is validated only on [298.15,1500] K");
    }
    if (component >=
        nist_ideal_gas_cp_j_per_mol_k.size()) {
        throw std::out_of_range(
            "mpmc::flow::NIST Cp component index out of range");
    }
...
/// Narrow production property provider for the repository-curated
/// methane/ethane/propane PR76 snapshot.
///
/// Caloric reference:
///   h_i^ig(298.15 K) = 0 for every component.
/// This is a legitimate nonreactive-flow reference convention; all accumulation,
/// advective enthalpy and boundary/well energy terms must use the same convention.
///
/// Ideal-gas sensible enthalpy uses piecewise-linear integration of NIST SRD 69
/// recommended Cp values on the common [298.15,1500] K interval.
///
/// Dense-fluid viscosity uses Lohrenz-Bray-Clark with:
/// - Stiel-Thodos dilute pure-component viscosity,
/// - Herning-Zipperer dilute-mixture blending,
/// - Kay pseudo-critical mixing,
/// - LBC/JST polynomial coefficient a5 = 0.0093324.
///
/// The Stiel-Thodos/LBC field-unit viscosity parameter is evaluated explicitly in
/// deg R, psia and lbm/lbmol, then viscosity is converted cP -> Pa s.
///
/// PR residual enthalpy uses the standard PR departure expression. da_mix/dT is
/// obtained by a one-direction nested Dual applied ONLY to the explicit PR mixing
/// kernel. The selected Z root remains on the established first-order IFT path;
/// nested AD is intentionally not applied to the implicit root solver.
class Pr76MethaneEthanePropanePropertyProvider {
public:
    ...
    Pr76MethaneEthanePropanePropertyProvider(
        ...
        if (parameters.dataset_id() !=
                pr76_mep_property_detail::
                    required_pr76_dataset_id ||
            parameters.components().size() !=
                pr76_mep_property_detail::
                    component_ids.size()) {
            throw std::invalid_argument(
                "mpmc::flow::PR76 methane/ethane/propane provider only supports the repository-curated ternary dataset");
        }
        ...
        for (std::size_t component = 0U;
            ...
            if (record.id !=
                ...
                throw std::invalid_argument(
                    "mpmc::flow::PR76 methane/ethane/propane component order mismatch");
            }
            if (!record.molar_mass ||
                record.molar_mass->unit !=
                    thermodynamics::Unit::
                        kilogram_per_mole) {
                throw std::invalid_argument(
                    "mpmc::flow::PR76 methane/ethane/propane provider requires sourced molar masses");
            }
            ...
            if (!std::isfinite(actual) ||
                std::abs(actual - expected) >
                    64.0 *
                        ...
                        scale) {
                throw std::invalid_argument(
                    "mpmc::flow::PR76 methane/ethane/propane molar-mass snapshot does not match the NIST SRD 69 provider record");
            }
        }
        ...
        return {
            {
                ...
                "DeitersBell-aic16730-PengRobinson1976-ternary__NIST-SRD69-MW",
                "v1"},
            {
                ...
                "a5=0.0093324-field-units-v1"},
            {
                "NIST SRD 69 recommended ideal-gas Cp + PR76 residual enthalpy",
                "NIST-SRD69-CH4-C2H6-C3H8-Cp__PR76-departure",
                "hig-298.15K-zero__v1"},
            {
    ...
    lbc_dynamic_viscosity_pa_s(
        ...
        for (std::size_t component = 0U;
            ...
            if (!std::isfinite(xi) ||
                !(xi > 0.0)) {
                throw std::range_error(
                    "mpmc::flow::invalid LBC pure-component viscosity parameter");
            }
    ...
    pr76_residual_molar_enthalpy_j_per_mol(
        const thermodynamics::Pr76SelectedPhase&
            selection,
...
private:
    const thermodynamics::Pr76Phase<double>*
        model_;
    thermodynamics::Pr76Mixture<double>
        mixture_;
    std::array<double, 3>
        critical_temperature_k_{};
    std::array<double, 3>
        critical_pressure_pa_{};
};
...
#endif // MPMC_FLOW_PR76_METHANE_ETHANE_PROPANE_PROPERTIES_HPP
