...
scan_post_snes_sw92_profile_c_source_cell_3d(
    ...
    *scan_status =
    ...
    try {
        ...
        if (accepted->phases.size() <
            source.source_phase_count) {
            // This SW92-specific boundary has just completed a fresh
            // boundary-aware Profile-C solve at the post-SNES p/T/feed and
            // authoritatively published the lower-cardinality topology.
            // That publication is the target-topology re-solve/review
            // evidence. Keep this source-cardinality-specific fact out of
            // the generic role-neutral backend, whose PtFlashRequest carries
            // no source phase count.
            mpmc::flow::PhaseSetTransitionCandidate lower;
