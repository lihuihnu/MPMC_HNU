#ifndef MPMC_FLOW_DISCRETIZATION_PETSC_TWO_PHASE_SNES_ASSEMBLY_HPP
#define MPMC_FLOW_DISCRETIZATION_PETSC_TWO_PHASE_SNES_ASSEMBLY_HPP

#include <mpmc/flow/two_phase_natural_variable.hpp>
#include <mpmc/flow_discretization/two_phase_tpfa.hpp>
...
#include <mpmc/flow_discretization_petsc/two_phase_fugacity_global_assembly_mapping.hpp>
...
struct TwoPhaseCurrentCellLinearization3D {
    mpmc::flow::NaturalVariableCellState2P state;
    mpmc::flow::
...
using TwoPhaseCurrentCellEvaluator3D =
    PetscErrorCode (*)(
...
struct TwoPhaseSnesCellInput3D {
    mpmc::mesh::LocalIndex cell{
...
struct TwoPhaseSnesAuthoritativeFaceInput3D {
...
struct TwoPhaseCurrentCellEvaluatorBinding3D {
    TwoPhaseCurrentCellEvaluator3D evaluator{};
    void* user_context{};
};

namespace two_phase_snes_assembly_detail {
...
same_layout(
    const mpmc::flow::NaturalVariableLayout2P& first,
    const mpmc::flow::NaturalVariableLayout2P& second) {
...
class TwoPhaseSnesAssemblyContext3D {
public:
    static constexpr std::string_view convention =
        two_phase_snes_assembly_convention;

    TwoPhaseSnesAssemblyContext3D(
        const TwoPhaseSnesAssemblyContext3D&) =
        delete;
    TwoPhaseSnesAssemblyContext3D& operator=(
        const TwoPhaseSnesAssemblyContext3D&) =
        delete;
    TwoPhaseSnesAssemblyContext3D& operator=(
        TwoPhaseSnesAssemblyContext3D&&) =
    ...
    TwoPhaseSnesAssemblyContext3D(
        TwoPhaseSnesAssemblyContext3D&& other) noexcept
        : comm_(other.comm_),
    ...
    create(
        ...
        std::vector<TwoPhaseSnesCellInput3D>
            cell_inputs,
        std::vector<
            TwoPhaseSnesAuthoritativeFaceInput3D>
            face_inputs,
        ...
        std::optional<
            ...
            output) {
        using namespace
            two_phase_snes_assembly_detail;
        ...
        try {
            if (cell_evaluator.evaluator == nullptr ||
                ...
                throw std::invalid_argument(
                    "two-phase SNES assembly metadata mismatch");
            }
            ...
            if (variable.location !=
                ...
                dof_layout.dofs_per_entity(
                    ...
                    q) {
                throw std::invalid_argument(
                    "two-phase natural-variable DoF block must be exactly 2*Nc+1 on cells");
            }
            component_count =
                (q - 1U) / 2U;
            if (component_count < 2U) {
                throw std::invalid_argument(
                    "two-phase SNES assembly requires at least two components");
            }
            ...
            for (std::size_t index = 0U;
                ...
                if (local >= cell_lookup.size() ||
                    ...
                    input.frozen_layout
                        ...
                        q) {
                    throw std::invalid_argument(
                        "invalid two-phase cell binding");
                }
                ...
                if (index == 0U) {
                    ...
                    canonical_component_ids) {
                    throw std::invalid_argument(
                        "two-phase component identity/order is not canonical across local overlap");
                }
                ...
                if (owned !=
                    ...
                    owned !=
                        input.previous_energy_accumulation
                            .has_value()) {
                    throw std::invalid_argument(
                        "owned two-phase cell history is required and ghost history is forbidden");
                }
                if (owned) {
                    if (input
                            .previous_component_accumulation
                        ...
                        !near_roundoff(
                            ...
                            input.porosity)) {
                        throw std::invalid_argument(
                            "two-phase previous accumulation does not match frozen cell identity");
                    }
                }
                cell_lookup[local] =
                    index;
            ...
            if (std::find(
                ...
                cell_lookup.end()) {
                throw std::invalid_argument(
                    "two-phase local cell bindings are incomplete");
            }
            ...
            for (std::size_t index = 0U;
                ...
                if (local >= face_lookup.size() ||
                    ...
                    input.thermal_conductance
                        ...
                        0.0) {
                    throw std::invalid_argument(
                        "invalid two-phase authoritative face binding");
                }
                face_lookup[local] =
                    index;
            }
            ...
            for (const auto& row :
                ...
                if (local >= face_lookup.size() ||
                    !face_lookup[local]
                         .has_value()) {
                    throw std::invalid_argument(
                        "two-phase authoritative schedule face lacks static binding");
                }
                const auto& input =
                    face_inputs[
                        *face_lookup[local]];
                if (input.face_global !=
                    ...
                    throw std::invalid_argument(
                        "two-phase face transmissibility does not match schedule");
                }
            }
        } catch (...) {
            local_error =
    ...
    evaluate_complete_assembly(
        ...
        *status =
        ...
        std::vector<
            OwnedCellTwoPhaseFugacityLinearizationBinding3D>
            fugacity_bindings;
    ...
    positive_support_precheck(
        ...
        try {
            for (std::size_t block = 0U;
                ...
                auto limit_positive =
                    [&](std::size_t slot) {
                        ...
                        if (!std::isfinite(value) ||
                            ...
                            !(value > 0.0)) {
                            throw std::invalid_argument(
                                "two-phase current state is outside positive support");
                        }
                        if (direction > 0.0 &&
                            value - direction <=
                                0.0) {
                ...
                const std::size_t saturation_slot =
                    layout.independent_saturation_unknown_index();
                const double s0 =
                    static_cast<double>(
                        PetscRealPart(
                            x[base + saturation_slot]));
                const double ds0_direction =
                    static_cast<double>(
                        PetscRealPart(
                            y[base + saturation_slot]));
                if (!std::isfinite(s0) ||
                    !std::isfinite(ds0_direction) ||
                    !(s0 > 0.0) ||
                    !(s0 < 1.0)) {
                    throw std::invalid_argument(
                        "two-phase saturation is outside positive support");
                }
                if (ds0_direction > 0.0 &&
                    s0 - ds0_direction <= 0.0) {
                    local_scale =
                        std::min(
                            local_scale,
                            0.8 * s0 /
                ...
                for (std::size_t phase = 0U;
                    ...
                    if (!std::isfinite(dependent) ||
                        ...
                        !(dependent > 0.0)) {
                        throw std::invalid_argument(
                            "two-phase dependent composition is outside positive support");
                    }
...
private:
    ...
    static void validate_current_cell(
        const TwoPhaseSnesCellInput3D& input,
        const TwoPhaseCurrentCellLinearization3D&
            current) {
        using namespace
            two_phase_snes_assembly_detail;
            ...
            !mpmc::flow::two_phase_detail::
                same_layout(
                    ...
                    input.frozen_layout
                        .descriptor())) {
            throw std::invalid_argument(
                "two-phase current-cell evaluator changed frozen chart/component identity");
        }
        ...
        if (!mpmc::flow::two_phase_detail::
                same_layout(
                    ...
                    input.frozen_layout
                        .descriptor()) ||
            current.fugacity.component_ids !=
                input.component_ids ||
            current.fugacity.residual_count() !=
                input.component_ids.size() ||
            current.fugacity.input_count !=
                input.frozen_layout
                    .unknown_count()) {
            throw std::invalid_argument(
                "two-phase fugacity linearization changed frozen chart/component identity");
        }
    }
...
#endif // MPMC_FLOW_DISCRETIZATION_PETSC_TWO_PHASE_SNES_ASSEMBLY_HPP
