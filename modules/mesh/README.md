...
- 在已通过的 processed GRDECL 3D serial topology + coordinate/metric gate 上，后续可单独建立 rooted `2×1×1` 3D DMPlex distribute/overlap gate，先验证 stable identity 与 coordinate migration，再决定是否迁移 processed property/face-geometry snapshots；仍不为调用原生 FVM geometry 而在 core 中强制物化 edge；
