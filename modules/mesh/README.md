...
> 当前状态：core topology/index、2D Cartesian topology/geometry、`FaceBoundarySnapshot`、`DenseFieldSnapshot`、`DofLayout`、`PartitionSnapshot`、`DofNumberingSnapshot` 与 `SharedEntityPlan` 已建立；Gmsh MSH 4.1 ASCII 与 VTU ASCII 已有 2D import/export round-trip，GRDECL 已有 raw corner-point parser，并新增无-fault active-cell face processor。processor 从 `GrdeclImportResult` 过滤 `ACTNUM=1` cells，按 exact corner coordinates 合并 shared vertices，再按 canonical 4-vertex face key 构造 `cell->vertex/cell->face/face->vertex/face->cell`；processed cell `GlobalEntityId` 与显式 source mapping 都保留原 logical cell ID。当前只验证无 fault 的 `2×1×1` 与 `1×1×2`，fault split/pinch/NNC 继续显式拒绝或后置。core 仍不依赖 PETSc/MPI；不含 VTU binary/appended/compressed、Gmsh binary/high-order/3D、GRDECL fault/NNC processing、Mat、残差、求解器或流动物理。
...
active face processor gate 使用两种正交共享方向：`2×1×1` 两 active cells 验证 I-interface，`1×1×2` 两 active cells 验证 K-interface；两者都必须从 16 个 cell-local corners 压成 12 unique vertices、从 12 张 cell-local faces 压成 11 faces，并且只有 1 张 `face->cell` support=2 的 shared face。相同 raw input 重复处理必须得到完全相同的 Topology 与 vertex ordering。activity mapping gate 将第一个 logical cell 设 inactive、只保留 logical cell 2，要求 processed local cell 0 的 topology cell ID 与 source mapping 都仍为 2，且 active-inactive interface 在 processed topology 中成为 boundary。另有 split-face fixture：两个 active logical neighbors 的共享 pillar face z 值故意错开，raw GRDECL geometry 仍可解析，但 processor 必须拒绝 unsupported fault split；全 inactive input 也拒绝，因为没有 computational cells 可输出。
...
- 在已验证的 active corner-point shared-face topology 上补 active-cell property projection：按 `source_logical_cell_ids` 将 PORO/PERMX/PERMY/PERMZ 过滤并重建为 processed-topology `DenseFieldSnapshot`，同时建立最小 quad-face metric（centroid/area/owner normal）；fault split/pinch/NNC 继续后置；
