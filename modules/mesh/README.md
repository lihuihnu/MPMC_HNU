...
- 在已通过的 rooted processed GRDECL 3D identity + coordinate + cell-property migration gate 上，下一步应单独迁移 `FaceGeometry3D`（centroid/area/owner/owner-relative normal），重点解决 face owner 在 repartition/ghost 视图中的 stable-cell 语义；仍不同时进入 transmissibility、Darcy flux 或 residual；
