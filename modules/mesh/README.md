...
- 在已通过的 rooted processed GRDECL 3D identity + coordinate + cell-property + stable-owner face-geometry migration gate 上，下一步建议先建立最小 3D cell/face geometric operator contract（只定义 cell-to-face displacement、normal distance 与 transmissibility 所需几何量的输入输出和退化拒绝规则），暂不引入 Darcy flux、岩石/流体物性组合或残差装配；
