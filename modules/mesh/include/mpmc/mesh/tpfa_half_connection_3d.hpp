#ifndef MPMC_MESH_TPFA_HALF_CONNECTION_3D_HPP
#define MPMC_MESH_TPFA_HALF_CONNECTION_3D_HPP
...
inline void require_tensor(
    ...
    if (!std::isfinite(tensor.kxx_m2) ||
        ...
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: permeability tensor must be finite and non-negative");
    }
}

inline void require_unit_normal(UnitVector3D normal) {
    if (!std::isfinite(normal.x) ||
        !std::isfinite(normal.y) ||
        !std::isfinite(normal.z)) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: outward normal must be finite");
    }
    ...
    if (!std::isfinite(magnitude) ||
        std::abs(magnitude - 1.0) >
            tolerance) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: outward normal must have unit length");
    }
}

inline void require_displacement(Displacement3D displacement) {
    if (!std::isfinite(displacement.x_m) ||
        !std::isfinite(displacement.y_m) ||
        !std::isfinite(displacement.z_m)) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: cell-to-face displacement must be finite");
    }
}
...
make_tpfa_half_connection_coefficient_3d(
    CartesianDiagonalPermeabilityTensor3D permeability,
    UnitVector3D outward_unit_normal,
    Displacement3D cell_to_face_displacement_m) {
    ...
    if (!std::isfinite(squared_distance_m2) ||
        ...
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: d^T d must be finite and strictly positive");
    }
    ...
    if (!std::isfinite(numerator_m3)) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: n^T K d must be finite");
    }
    ...
    if (!std::isfinite(projection_scale_m3)) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: permeability/displacement scale must be finite");
    }
    ...
    if (numerator_m3 <
        ...
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: n^T K d is negative under the outward-normal/cell-to-face sign convention");
    }
    ...
    if (!std::isfinite(coefficient_m) ||
        coefficient_m <= 0.0) {
        throw std::invalid_argument(
            "mpmc::mesh::make_tpfa_half_connection_coefficient_3d: positive projection must produce a finite positive coefficient");
    }
...
make_owner_tpfa_half_connection_coefficient_3d(
    ...
    if (geometry.cell_count() !=
        ...
        throw std::invalid_argument(
            "mpmc::mesh::make_owner_tpfa_half_connection_coefficient_3d: geometry/permeability cell counts do not match");
    }
...
make_neighbour_tpfa_half_connection_coefficient_3d(
    ...
    if (geometry.cell_count() !=
        permeability.cell_count()) {
        throw std::invalid_argument(
            "mpmc::mesh::make_neighbour_tpfa_half_connection_coefficient_3d: geometry/permeability cell counts do not match");
    }
    ...
    if (!neighbour.has_value() ||
        ...
        throw std::invalid_argument(
            "mpmc::mesh::make_neighbour_tpfa_half_connection_coefficient_3d: internal neighbour half connection is required");
    }
...
#endif // MPMC_MESH_TPFA_HALF_CONNECTION_3D_HPP
