...
void printed_reference() {
    ...
    // Independent Decimal(70-digit) evaluation of paper equations, using EXACT
    // decimal Tc=400, Pc=4000000, omega=0.2 and modern SI R. These are not data
    // copied from the paper's tables nor values generated by the tested C++ code.
    near(pure.critical_attraction(), 1.26436532690287949346109988523210496L);
    near(pure.covolume(), 0.0000646865191692322072L);
    near(pure.kappa(), 0.6722952L);
    ...
    const std::array<long double, 3> values{
        2.25725929157385587190378283570899166L,
        1.26436532690287949346109988523210496L,
        0.55720581092738237263912515658480156L};
    const std::array<long double, 3> slopes{
        -0.005678805945093580036265185306958392L,
        -0.002125066850808091874080822098905238L,
        -0.000940487152712929153352701029554186L};
...
void analytic_temperature() {
    ...
    // omega>0.49 deliberately still uses the PR76 polynomial, NOT PR78.
    near(kernel<T>(400, 4e6, 0.6).kappa(), 1.2028248L);
}
...
void numerical_range() {
    ...
    for (double scale : scales) {
        ...
        near(result.a.derivative(0), -factor * 0.6722952L);
