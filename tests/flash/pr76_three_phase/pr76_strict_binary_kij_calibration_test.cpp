...
th::Provenance leal_source(std::string locator) {
    return {
        ...
        "M. F. Leal et al., Fluid Phase Equilibria 478 (2018)",
        std::move(locator),
...
th::Provenance trial_kij_source(const std::string& pair_id) {
    return {
        ...
        "strict-PR76-binary-kij-calibration",
        "bounded-1D-fit-v1",
        "optimization variable for " + pair_id,
        ...
        "Test-only calibration variable"};
}
...
// Pure-component inputs are deliberately held fixed to the same UFC Table 6
// contract intended for the later blind ternary validation. The binary data,
// not a transferred literature kij, determine the strict-PR76 interaction.
constexpr HeavyComponent decane{
    "n-decane", 617.7, 2.110e6, 0.4923};
constexpr HeavyComponent hexadecane{
    "n-hexadecane", 717.0, 1.489e6, 0.742};
constexpr double co2_tc_k = 304.21;
constexpr double co2_pc_pa = 7.383e6;
constexpr double co2_omega = 0.2236;

// UFC 2025 Table 7: independent CO2+n-decane bubble points near 323.15 K.
// The two near-critical highest-x points are intentionally excluded from this
// local fit so candidate phase-role coalescence cannot dominate a one-parameter
// regression. No ternary observation is present in this dataset.
...
// Leal et al. 2018 Table 5. The same three prepared liquid compositions were
// measured at 313.2 and 333.2 K. These observations are independent of the UFC
// ternary campaign. U(p)=0.01 MPa, u(T)=0.1 K and u(x_CO2)=0.0003 are reported.
...
std::optional<double> transition_pressure(
    ...
    // The fit bounds keep the expected bubble locus close to the observations.
    // Starting close avoids accidentally bracketing a distant low-pressure dew
    // transition for CO2-rich samples.
    double lower = 0.82 * datum.pressure_pa;
    double upper = 1.22 * datum.pressure_pa;
    auto lower_status = classify_feed(lower, datum, evaluator);
    auto upper_status = classify_feed(upper, datum, evaluator);
    ...
    // A 0.02 MPa-scale pressure resolution is already finer than needed for a
    // one-parameter regression against these datasets, while keeping the hosted
    // calibration inexpensive and deterministic.
...
std::optional<FitEvaluation> evaluate_kij(
    const HeavyComponent& heavy, double kij,
...
template <std::size_t N>
FitEvaluation fit_kij(
    const HeavyComponent& heavy, const std::array<BinaryDatum, N>& data) {
    // Wide physical search interval; no literature-fitted kij is used as an
    // initial guess or target. Invalid candidates simply receive a large cost.
    double left = 0.0;
    double right = 0.16;
...
void print_fit(std::string_view name, const FitEvaluation& fit,
    ...
    for (std::size_t i = 0; i < N; ++i) {
        std::cout << "  T=" << data[i].temperature_k
                  << " xCO2=" << data[i].x_co2
...
void calibrate_independent_binary_kij() {
    ...
    const double weight =
        (target_temperature_k - low_temperature_k) /
        (high_temperature_k - low_temperature_k);
    const double c16_323 =
        c16_313.kij + weight * (c16_333.kij - c16_313.kij);
    ...
    // Scientific acceptance is about a reproducible, bounded binary regression,
    // not matching the dissertation's transferred high-omega-PR parameters.
    require(c10.kij > 0.0 && c10.kij < 0.16,
            "CO2+n-C10 strict-PR76 fitted kij left the predeclared search interval");
    ...
    // Guard the data-provenance separation that motivated this calibration.
    require(decane_323_data.size() == 4U &&
                hexadecane_313_data.size() == 3U &&
