...
void wet_feed_finds_water_phase() {
    ...
    require(result.retained_h_phase_count() == 2U,
            "wet fixed-NA source did not retain its accepted H pair");
    const auto* witness = result.selected_water_witness();
    require(witness->usable_w_seed() && witness->water_role_admissible &&
                witness->distinct_from_every_retained_h &&
                witness->point.composition[result.water_index] >
                    result.max_retained_h_water_fraction,
            "selected W witness failed role/distinction guards");
...
void dry_feed_closes_single_h() {
    ...
    require(result.status ==
                ...
                result.no_w_locally_closed() && result.retained_h_phase_count() == 1U &&
                result.aqueous_appearance_search.has_value() &&
                result.selected_water_witness() == nullptr,
            "dry feed did not close as one unresolved H phase under targeted AQ review");
...
void zero_water_closes_by_inventory() {
    ...
    require(result.no_w_locally_closed() &&
                result.retained_h_phase_count() == 1U &&
...
void witness_guards() {
    ...
    const auto duplicate_result = fl::detail::sw92_phase_assigned_classify_water_witness(
        0U, duplicate, retained, feed, 1U, 0.9, 1e-7);
    require(!duplicate_result.distinct_from_every_retained_h &&
                !duplicate_result.usable_w_seed(),
            "retained H composition was misclassified as a W seed");

    fl::TpdPoint water_poor;
    water_poor.composition = {0.2, 0.8};
    water_poor.value = -1.0;
    const auto rejected = fl::detail::sw92_phase_assigned_classify_water_witness(
        1U, water_poor, retained, feed, 1U, 0.9, 1e-7);
    require(rejected.distinct_from_every_retained_h &&
                !rejected.water_role_admissible && !rejected.usable_w_seed(),
            "water-poor AQ mathematical negative passed the physical W-role guard");
    ...
    const auto usable = fl::detail::sw92_phase_assigned_classify_water_witness(
        2U, water_rich, retained, feed, 1U, 0.9, 1e-7);
    require(usable.distinct_from_every_retained_h &&
                usable.water_role_admissible && usable.usable_w_seed(),
...
void component_permutation() {
    ...
    require(normal.retained_h_phase_count() == reverse.retained_h_phase_count(),
            "component permutation changed retained H phase count");
    near(normal.max_retained_h_water_fraction,
         reverse.max_retained_h_water_fraction);
