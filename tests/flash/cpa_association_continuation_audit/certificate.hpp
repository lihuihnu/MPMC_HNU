...
inline Result certify(
    ...
    // The fixed-point coefficients must describe the exact same numerical
    // operator that production iterates. Association strengths are therefore
    // generated by the production double-precision kernel first and only then
    // lifted to long double for the enclosure algebra. Recomputing Delta in
    // long double would certify a subtly different fixed-point equation.
    for (std::size_t a = 0U; a < n; ++a) {
        const auto& first = association.sites[a];
        for (std::size_t b = 0U; b < n; ++b) {
            ...
            const double delta_double = th::cpa_detail::cpa_association_strength(
                temperature_k, association.radial_distribution, b_ij, *pair);
            if (!std::isfinite(delta_double) || delta_double < 0.0) { return result; }
            const Real delta = static_cast<Real>(delta_double);
