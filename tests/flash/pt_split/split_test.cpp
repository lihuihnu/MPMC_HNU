...
void rr_zero_trace() {
    for (double trace : {0.0,1e-250}) {
        ...
        if (trace==0) { require(r.liquid[2]==0 && r.vapor[2]==0,"zero component added"); }
        else {
            // Returned fractions include the explicitly recorded roundoff-only
            // normalization. Compare RELATIVELY to the nonzero trace, not bitwise
            // or using an absolute floor that could hide a vanished component.
            const double eps=std::numeric_limits<double>::epsilon();
            require(r.liquid[2]>0 && r.vapor[2]>0,"trace floored or lost");
            require(std::abs(r.liquid[2]/trace-1)<=128*eps &&
                    std::abs(r.vapor[2]/trace-1)<=128*eps,"trace normalization exceeds roundoff bound");
            require(std::abs((r.liquid[2]/trace)*r.raw_liquid_sum-1)<=8*eps &&
                    std::abs((r.vapor[2]/trace)*r.raw_vapor_sum-1)<=8*eps,"raw K=1 trace identity");
        }
...
void iterate_callbacks() {
    ...
    // Runtime fault callbacks keep the provider contract capable of returning a
    // phase; a statically always-throwing template argument causes MSVC C4702 at
    // the caller's post-call validation. Do not suppress production warnings.
    using FaultProvider=std::function<fl::PtSplitPhase(double,double,std::span<const double>,fl::PtPhaseRole)>;
    const FaultProvider fails=[](double,double,std::span<const double>,fl::PtPhaseRole)->fl::PtSplitPhase {
    ...
    const FaultProvider bug=[](double,double,std::span<const double>,fl::PtPhaseRole)->fl::PtSplitPhase {throw std::domain_error("input error");};
...
void gibbs_slope() {
    // Analytic RR state: z=(1/2,1/2), beta=1/2, x=(1/3,2/3), y=(2/3,1/3).
    // c=(4/9,4/9), H=4/9. These exact rational identities independently fix
    // both the diagonal and the beta-coupling terms of the directional derivative.
    fl::PtSplitState s;
    s.fractions.liquid={1.0/3,2.0/3}; s.fractions.vapor={2.0/3,1.0/3};
    s.fractions.vapor_fraction=.5;
    const double a=std::log(2.0);
    s.fugacity_residual={-a,a};
    const long double a_ref=std::log(2.0L);
    near(fl::detail::split_gibbs_slope(s,Vec{.5,.5},1),-2.0L*a_ref*a_ref/9);
    near(fl::detail::split_gibbs_slope(s,Vec{.5,.5},2),-a_ref*a_ref/9);
    s.fugacity_residual={a,a};
    near(fl::detail::split_gibbs_slope(s,Vec{.5,.5},1),-2*a_ref*a_ref);
    s.fractions.vapor=s.fractions.liquid;
    require(!std::isfinite(fl::detail::split_gibbs_slope(s,Vec{.5,.5},1)),"degenerate slope invented");
}
void gibbs_roundoff() {
    fl::PtSplitState old_point, next;
    old_point.reduced_gibbs=1; old_point.gibbs_roundoff_guard=1e-12; old_point.fugacity_norm=1e-7;
    next=old_point; const fl::PtSplitIterationOptions options;
    require(!fl::detail::split_accept_step(old_point,next,1e-18,1,options),"unresolved no-op accepted");
    next.fugacity_norm=2e-7;
    require(!fl::detail::split_accept_step(old_point,next,1e-18,1,options),"unresolved growing residual accepted");
    next.fugacity_norm=5e-8;
    require(fl::detail::split_accept_step(old_point,next,1e-18,1,options),"resolved residual progress rejected");
    next.reduced_gibbs=.9; next.fugacity_norm=2e-7;
    require(fl::detail::split_accept_step(old_point,next,.2,1,options),"Gibbs descent with temporary residual increase rejected");
    next.reduced_gibbs=1.1;
    require(!fl::detail::split_accept_step(old_point,next,.2,1,options),"Gibbs ascent accepted");
}
void pr_gibbs_progress() {
    const auto model=literature_model(false); fl::Pr76VleEvaluator evaluator(model);
    const Vec z{.3,.7};
    const auto seed=fl::detail::split_seed(z,Vec{.5,.5},true);
    require(seed.has_value(),"regression seed");
    const auto r=fl::iterate_pt_split(7.6e6,270,z,*seed,evaluator);
    require(r.status==fl::PtSplitAttemptStatus::converged && r.point,"Gibbs-protected SSI stalled");
    require(r.residual_increase_steps>0 && r.gibbs_descent_steps>0,"nonmonotone residual path not exercised");
    require(r.point->fugacity_norm<=1e-11,"Gibbs decrease substituted for equilibrium");
    near(r.point->fractions.vapor_fraction,golden[1][2],2e-9);
    std::cout<<"Gibbs steps="<<r.gibbs_descent_steps<<" residual increases="<<r.residual_increase_steps<<'\n';
}
void result_guard() {
    fl::PtSplitResult r;
    r.selected_attempt=0;
    require(!r.candidate() && !r.equations_converged(),"invalid public index unchecked");
    r.attempts.resize(1); r.attempts[0].status=fl::PtSplitAttemptStatus::converged;
    require(!r.candidate(),"empty public point unchecked");
    r.attempts[0].point.emplace();
    require(r.candidate()==&*r.attempts[0].point,"valid point inaccessible");
    r.attempts[0].status=fl::PtSplitAttemptStatus::iteration_limit;
    require(!r.candidate() && !r.equations_converged(),"unconverged public attempt exposed");
    r.selected_attempt=std::numeric_limits<std::size_t>::max();
    require(!r.candidate(),"oversized public index unchecked");
}
...
constexpr Test tests[]={
    ...
    {"gibbs_slope",gibbs_slope},{"gibbs_roundoff",gibbs_roundoff},
    {"pr_gibbs_progress",pr_gibbs_progress},{"result_guard",result_guard}
};
} // namespace
