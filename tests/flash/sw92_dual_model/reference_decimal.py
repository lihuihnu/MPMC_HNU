"""Independent Decimal(80) SW92 Whitson dual-model observable anchors.

Stdlib only; no production imports. Raw CO2/CH4/water properties and
non-aqueous BIPs are transcribed from Soreide-Whitson 1992 Tables 3/5 and the
corrected original AQ correlations. Each state is solved twice: once with the
AQ family and once with the NA family. The aqueous target is the water-richer
phase of the AQ run; the non-aqueous target is the water-poorer phase of the NA
run. The reported ratio is x_NA_target / x_AQ_target componentwise.

These are compatibility-model numerical anchors, not experimental measurements,
not a common material balance, and not a joint AQ/NA Gibbs-equilibrium proof.
"""
...
def solve_family(gas: str, family: str, pressure: D, temperature: D,
                 molality: D, feed: D, x_seed: str, y_seed: str):
    x = D(x_seed)
    y = D(y_seed)
...
def solve_case(gas: str, pressure_value: str, temperature_value: str,
               molality_value: str, feed_value: str,
               aq_x_seed: str, aq_y_seed: str, na_x_seed: str, na_y_seed: str):
    pressure = D(pressure_value)
    temperature = D(temperature_value)
    molality = D(molality_value)
    feed = D(feed_value)
    aq = solve_family(gas, "AQ", pressure, temperature, molality, feed,
                      aq_x_seed, aq_y_seed)
    na = solve_family(gas, "NA", pressure, temperature, molality, feed,
                      na_x_seed, na_y_seed)
    aq_target = [aq[0], D(1) - aq[0]]
    na_target = [na[1], D(1) - na[1]]
    ratio = [na_target[i] / aq_target[i] for i in range(2)]
    return aq + na + (ratio[0], ratio[1])


def anchors():
    return [
        solve_case("CO2", "3e6", "340", "0", "0.7",
                   "0.006", "0.987", "0.00028", "0.989"),
        solve_case("CH4", "1e7", "350", "1", "0.5",
                   "0.0009", "0.9904", "0.000006", "0.9947"),
    ]
...
if __name__ == "__main__":
    with localcontext() as context:
        context.prec = 80
        computed = anchors()
        text = Path(__file__).with_name("sw92_dual_model_test.cpp").read_text(encoding="utf-8")
        block = text.split("constexpr DualGolden golden[] = {", 1)[1].split("};", 1)[0]
        ...
        if len(literals) != expected_count:
            raise AssertionError(f"SW92 dual-model anchor count changed: {len(literals)} != {expected_count}")
        cursor = 0
        for index, values in enumerate(computed):
            for value in values:
                ...
                if abs(literal - value) > D("1e-32") * max(D(1), abs(value)):
                    raise AssertionError(f"SW92 dual-model anchor mismatch: {literal} vs {value}")
            print("SW92 dual-model case", index + 1,
                  "AQ x=", format(values[0], ".35g"),
                  "NA y=", format(values[4], ".35g"),
                  "ratio_gas=", format(values[6], ".35g"))
        print(f"Independent Decimal(80) SW92 dual-model anchors: {len(computed)}/{len(computed)} checked")
